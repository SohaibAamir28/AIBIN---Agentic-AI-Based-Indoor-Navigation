"""
Category service for AIBIN Indoor Navigation platform.
...
class CategoryService:
    """Service for category operations following AIBIN async patterns."""
    ...
    async def delete_category(
        ...
        Raises:
            ValueError: If category has Projects or child categories
        """
        # Get category
        category = await self.get_category_by_id(category_id)
        if not category:
        ...
        if product_count > 0:
            raise ValueError("Cannot delete category with Projects. Move Projects to another category first.")
